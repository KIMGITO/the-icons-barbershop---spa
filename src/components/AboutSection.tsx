...
export const AboutSection: React.FC<AboutSectionProps> = ({ isStandalonePage = false }) => {
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="space-y-4">
                ...
                <div className="p-4 bg-card border border-border rounded-xl">
                  <div className="text-lg font-display font-bold text-primary">Hospital Grade Care</div>
                  <div className="text-xs text-muted-foreground font-light mt-0.5">Autoclaved machine detailing blades & hypoallergenic foil shavers</div>
                </div>
              </div>
              ...
              <div className="space-y-4 pt-6 sm:pt-10">
                <div className="p-4 bg-card border border-border rounded-xl">
                  <div className="text-lg font-display font-bold text-white">{businessInfo.address.suite}</div>
                  <div className="text-xs text-muted-foreground font-light mt-0.5">{businessInfo.address.street}, {businessInfo.address.city}</div>
                </div>
  ...
  <h2 className="font-display text-2xl sm:text-3xl lg:text-4xl font-bold text-white tracking-tight leading-snug">
    A Bespoke Sanctuary at <span className="text-primary">{businessInfo.address.suite}</span>
  </h2>

  <p className="text-xs sm:text-sm md:text-base text-muted-foreground-light leading-relaxed font-light">
    Located along {businessInfo.address.street}, <strong className="text-white font-medium">{businessInfo.name}</strong> redefines executive grooming combining precision machine craft with restorative scalp care in a quiet, high end haven.
  </p>
  ...
  <div className="pt-1 w-full flex justify-center ">
    <Button
      ...
      className="font-bold text-xs uppercase tracking-wider shadow-lg"
    >

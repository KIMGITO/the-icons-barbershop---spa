...
export const PortalOverview: React.FC<PortalOverviewProps> = ({
  ...
  // Operational metrics
  const pendingCount = relevantBookings.filter(
    (b) => b.status === 'pending',
  ).length;
  const todayTotalRevenue = todayBookings.reduce(
    (sum, b) => sum + (b.depositPaidKsh || 0),
    0,
  );
  const nextClient = todayBookings.find(
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            <div className="space-y-0.5">
              <h3 className="text-base font-bold text-foreground">
                {nextClient.customerName}
              </h3>
              <p className="text-xs text-muted-foreground flex items-center gap-2">
                <span>{nextClient.serviceNames.join(' + ')}</span>
                <span>•</span>
                <span>
                  Assigned: <strong>{nextClient.providerName}</strong>
                </span>
              </p>
            </div>

...
import { 
  ...
  Search, AlertTriangle, Save, X
} from 'lucide-react';
import { categoryService, ServiceCategoryRow } from '../../../services/categoryService';
...
export const ServiceCategoryManagement: React.FC = () => {
  ...
  const handleOpenEdit = (cat: ServiceCategoryRow) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingCategory) {
        ...
        addToast({ type: 'success', title: 'Success', message: 'Service category updated' });
      } else {
        ...
        addToast({ type: 'success', title: 'Success', message: 'Service category created' });
      }
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm('Delete this service category? Services in this category might become un-categorized.')) return;
    try {
      await categoryService.deleteCategory(id);
      addToast({ type: 'success', title: 'Success', message: 'Service category deleted' });
      loadCategories();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-4 pt-4 border-t border-border">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
        {loading ? (
          <div className="col-span-full py-12 flex flex-col items-center justify-center bg-card border border-border rounded-none text-muted-foreground">
          ...
          filteredCategories.map((cat) => (
            <div 
              key={cat.id} 
              className="bg-card border border-border hover:border-primary/40 rounded-none p-3 flex flex-col justify-between space-y-3 transition-all group"
            >
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2 min-w-0">
                  <div className={`shrink-0 w-8 h-8 rounded-none flex items-center justify-center ${cat.is_active ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>
              ...
              <div className="flex items-center justify-end gap-1.5 pt-2 border-t border-border/50">
                ...
                <button
                  ...
                  className="p-1.5 rounded-none text-muted-foreground hover:text-destructive hover:bg-muted transition-colors"
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/80 backdrop-blur-xs">
          <div className="relative w-full max-w-md bg-card border border-border rounded-none shadow-2xl p-6 space-y-4">
            ...
            {error && (
              <div className="p-3 bg-destructive/10 border border-destructive/30 rounded-none text-[11px] text-destructive flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1">
                  ...
                  <ThemeSelect
                    ...
                    className="w-full bg-input border border-border rounded-none px-3 py-2 text-xs"
                  >

...
async function sendAtSms(phone: string, message: string) {
  ...
  if (!AT_API_KEY) {
    return { delivered: 'failed' as const, errorMsg: 'AFRICASTALKING_API_KEY not configured' };
  }
  ...
  try {
    ...
    return { delivered, messageId: mid, errorMsg: delivered === 'failed' ? JSON.stringify(data) : null };
  } catch (err) {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!bookingId) {
      return Response.json({ error: 'bookingId is required' }, { status: 400, headers: corsHeaders });
    }
    ...
    // 3. Build Admin SMS
    const adminMsg = [
      `NEW BOOKING: ${booking.receipt_code}`,
    ...
    return Response.json({ success: true, customerStatus: customerResult.delivered }, { headers: corsHeaders });

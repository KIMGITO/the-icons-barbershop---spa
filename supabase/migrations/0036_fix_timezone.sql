-- ============================================================
-- 76. FIX: booking engine timezone bug
...
begin
  ...
  -- wrongly included in, the wrong day's window)
  for v_win in
...
-- ------------------------------------------------------------
-- get_available_slots: unchanged from 0017 — it already just
-- delegates to fn_get_staff_free_windows, so it inherits the fix
-- above automatically. Re-declared here only so the migration is
-- self-contained and explicit about what depends on the fix.
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
-- check_and_reserve: fix the business-hours check to compare in
-- Nairobi wall-clock time, and resolve the correct Nairobi
-- calendar date for slot lookup (fixes late-night bookings
-- resolving to the wrong day). Everything below the business-hours
-- check — staff assignment, pricing, booking creation — is
-- unchanged from 0017 except swapping `p_desired_start_ts::date`
-- for the new `v_desired_local_date` variable.
-- ------------------------------------------------------------
...
begin
  ...
  -- Resolve the Nairobi-local calendar date for this instant — a
  -- booking at, say, 00:30 Nairobi time is 21:30 UTC the previous
  -- day, so a plain ::date cast (session tz = UTC) would resolve
  -- to the wrong day.
  v_desired_local_date := (p_desired_start_ts at time zone 'Africa/Nairobi')::date;

  -- Check business hours — compare using Nairobi wall-clock time,
  -- not the session's (UTC) timezone.
  if not exists (
    select 1 from public.business_hours bh
    where bh.business_id = '00000000-0000-0000-0000-000000000001'
      and bh.weekday = extract(dow from (p_desired_start_ts at time zone 'Africa/Nairobi'))::int
      and bh.is_open
      and bh.open_time <= (p_desired_start_ts at time zone 'Africa/Nairobi')::time
      and bh.close_time >= (v_desired_end at time zone 'Africa/Nairobi')::time
  ) then
...
-- No new grants needed — `create or replace function` preserves the
-- existing grants from migration 0017 on all three functions.